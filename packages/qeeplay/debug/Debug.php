...
namespace qeeplay\debug;

/**
 * 用于帮助开发者对 QeePlay 调试的工具类
 */
abstract class Debug
{
    ...
    static function dump($vars, $label = null, $depth = null)
    {
        ...
        if ($trace[0]['function'] == 'dump' && $trace[0]['class'] == 'qeeplay\\debug\\Debug')
        {
            array_shift($trace);
        }
        $last = array_shift($trace);
        if (val($last, 'class') == 'qeeplay\\debug\\Debug' && val($last, 'function') == 'dump')
        {
            $last = array_shift($trace);
        }
