...
abstract class BaseAction
{
    ...
    function execute_wrapper()
    {
        ...
        if ($this->_validate_input())
        {
            ...
            if (!$this->_validate_output())
            {
                $this->_on_validate_output_failed();
            }
        }
        else
        {
            $this->_on_validate_input_failed();
        }
        $this->_after_execute();
    }
     ...
     *
     * 如果返回 false 则阻止调用 execute() 方法，并调用 _on_validate_input_failed() 方法。
     *
     * @return bool
     */
     ...
     *
     * 如果返回 false 则调用 _on_validate_output_failed() 方法。
     *
     * @return bool
     */
